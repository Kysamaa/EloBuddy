...
namespace AddonTemplate.Utility
{
    ...
    public static class Prediction2
    {
        ...
        internal static PredictionOutput GetPrediction(PredictionInput input, bool ft, bool checkCollision)
        {
            ...
            if (!input.Unit.IsValidTarget(false, range: float.MaxValue))
            {
            ...
            foreach (var enemy in
                EntityManager.Heroes.Enemies.FindAll(
                    h =>
                        ...
                        h.IsValidTarget(true, input.RangeCheckFrom, (input.Range + 200 + input.RealRadius))))
            {
                input.Unit = enemy;
                        ...
                        case CollisionableObjects.Heroes:
                            foreach (var hero in
                                EntityManager.Heroes.Enemies.FindAll(
                                    hero =>
                                        hero.IsValidTarget(
true, input.RangeCheckFrom, Math.Min(input.Range + input.Radius + 100, 2000)))
                                )
                            {
                                input.Unit = hero;
                                var prediction = Prediction2.GetPrediction(input, false, false);
