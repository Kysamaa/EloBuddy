...
namespace AddonTemplate.Utility
{
    ...
    public static class Utility2
    {
        ...
        {
            return
                ObjectManager.Get<Obj_AI_Turret>().Any(turret => turret.IsValidTarget(enemyTurretsOnly, position, 950));
        }
        ...
        {
            return EntityManager.Heroes.Enemies.Count(h => h.IsValidTarget(true, point, range));
        }
        ...
        public static int CountAlliesInRange(this Vector3 point, float range, Obj_AI_Base originalunit = null)
        {
            if (originalunit != null)
            {
                return EntityManager.Heroes.Allies
                    .Count(x => x.NetworkId != originalunit.NetworkId && x.IsValidTarget(false, point, range));
            }
            return EntityManager.Heroes.Allies
             .Count(x => x.IsValidTarget(false, point, range));
        }
